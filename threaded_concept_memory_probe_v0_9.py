...
"""
Threaded Concept Memory Probe v0.9
...
DEFAULT_DB_PATH = "threaded_concept_memory_probe_v0_9.db"
...
class ThreadedConceptMemoryStore:
    ...
    def get_fatigue(self, word: str, recent_turns: int = 10) -> int:
        next_turn = self.get_next_turn_no()
        min_turn = max(1, next_turn - recent_turns)
        row = self.conn.execute(
            "SELECT COUNT(*) AS c FROM conversation_exposures WHERE word = ? AND turn_no >= ?",
            (word, min_turn),
        ).fetchone()
        return int(row["c"]) if row else 0

    def record_exposures(self, gated: GatedContext, turn_no: Optional[int] = None) -> int:
        if turn_no is None:
        ...
        for gw in gated.words:
            if gw.suppressed_by_fatigue:
                continue
            cur.execute(
                "INSERT INTO conversation_exposures(id, word, thread_id, exposed_at, turn_no) VALUES (?, ?, ?, ?, ?)",
                ("exp_" + uuid.uuid4().hex[:12], gw.word, None, now, turn_no),
            )
        for th in gated.threads:
            cur.execute(
                "INSERT INTO conversation_exposures(id, word, thread_id, exposed_at, turn_no) VALUES (?, ?, ?, ?, ?)",
                ("exp_" + uuid.uuid4().hex[:12], None, th.thread_id, now, turn_no),
            )
        self.conn.commit()
        return turn_no
...
class ActivationGate:
    ...
    def gate(self, activation: ActivationResult) -> GatedContext:
        ...
        def fatigue_for(word: str) -> int:
            if self.store is None:
                return 0
            return self.store.get_fatigue(word, self.fatigue_recent_turns)
        ...
        gated_threads: list[GatedThread] = []
...
def build_response_prompt(user_input: str, activation: ActivationResult, include_trace: bool = False, trace_limit: int = 10, gated: Optional[GatedContext] = None) -> str:
    ...
    if gated is not None:
        ...
        lines.append("")
        lines.append("[Gated Experience Threads]")
        for th in gated.threads:
            ...
            lines.append(
                f"{th.thread_id} score={th.score:.3f} base={th.base_score:.3f}{bonus} key={th.canonical_key} count={th.same_key_thread_count} eff={th.effective_strength:.3f} direct={direct} core={core} support={support}"
            )
...
def print_activation(result: ActivationResult, trace_limit: int = 80) -> None:
    ...
    for th in result.activated_threads:
        ...
        print(f"- {th.thread_id} score={th.score:.4f} base={th.base_score:.4f}{bonus} matched={matched} activated_by={by} date={th.date}: {' / '.join(th.words)}")
...
def print_gated_context(gated: GatedContext) -> None:
    ...
    for th in gated.threads:
        ...
        bonus = f" bonus={th.common_bonus:.4f}" if th.common_bonus > 0 else ""
        print(
            f"- {th.thread_id} score={th.score:.4f}{bonus} key={th.canonical_key} "
            f"same_key_thread_count={th.same_key_thread_count} effective_strength={th.effective_strength:.4f} "
            f"direct={direct} core={core} support={support}"
        )
    ...
    if not gated.words:
        print("- (none)")
    for gw in gated.words:
        print(f"- {gw.word} score={gw.score:.4f} depth={gw.depth} role={gw.role} fatigue={gw.fatigue}")

    if gated.suppressed_words:
        print("\n[Suppressed Words]")
        for sw in gated.suppressed_words[:12]:
            print(f"- {sw.word} score={sw.score:.4f} depth={sw.depth} fatigue={sw.fatigue} suppressed={sw.suppressed_by_fatigue} reason={sw.reason}")
...
def cmd_ask(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if gate is not None:
            print_gated_context(gate.gate(result))
        if not args.no_reinforce:
            store.reinforce_seen(result.activated_words)
        if not args.no_response:
            print("\n[Response]")
            print(generator.generate(args.text, result, args.response_include_trace, args.response_trace_limit, gate=gate))
        if gate is not None:
            store.record_exposures(gate.gate(result))
        return 0
    finally:
        store.close()
    ...
    try:
        ...
        for t in tests:
            ...
            if args.seed_generate_response:
                print("\n[Response]")
                print(generator.generate(t, result, args.response_include_trace, args.response_trace_limit, gate=gate))
            if gate is not None:
                store.record_exposures(gate.gate(result))
            else:
                print("\n[Response]")
    ...
    try:
        ...
        while True:
            ...
            if mode in {"ask", "ask_then_add"}:
                ...
                store.reinforce_seen(result.activated_words)
                print("\n[Response]")
                print(generator.generate(text, result, args.response_include_trace, args.response_trace_limit, gate=gate))
                if gate is not None:
                    store.record_exposures(gate.gate(result))
